...
import type { MasterWorker } from "@/lib/types";
import { useMasterWorkers } from "@/hooks/useAsyncStorage";
...
interface MasterWorkersSectionProps {
...
export function MasterWorkersSection({ workers }: MasterWorkersSectionProps) {
    const { add, update, delete: remove, isAdding, isUpdating, isDeleting } = useMasterWorkers();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (form.id) {
                ...
                toast({ title: "Master worker updated successfully" });
            } else {
                await add(worker);
                toast({ title: "Master worker added successfully" });
            }
            resetForm();
        } catch (error) {
            toast({
                title: "Error",
                description: "Failed to save master worker.",
                variant: "destructive"
            });
        }
    };
    ...
    const columns = useMemo<ColumnDef<MasterWorker>[]>(() => [
        {
            accessorKey: "name",
            header: ({ column }) => (
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h2 className="text-xl font-semibold">Master Workers (Fixed Monthly Salary)</h2>
            ...
            {showForm && (
                <Card>
                    <CardHeader>
                        <CardTitle>{form.id ? "Edit Worker" : "Add New Master Worker"}</CardTitle>
                    </CardHeader>
                    <CardContent>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Master Worker List</CardTitle>
                </CardHeader>
                <CardContent>
