...
import type { TFOWorker, TFOAttendance } from "@/lib/types";
import { useTFOWorkers, useTFOAttendance } from "@/hooks/useAsyncStorage";
...
interface TFOWorkersSectionProps {
    workers: TFOWorker[];
    attendance: TFOAttendance[];
}
...
export function TFOWorkersSection({ workers, attendance }: TFOWorkersSectionProps) {
    const { add: addWorker, update: updateWorker, delete: removeWorker, isAdding: iaw, isUpdating: iuw, isDeleting: idw } = useTFOWorkers();
    const { add: addAttendance, delete: removeAttendance, isAdding: iaa, isDeleting: ida, refresh: refreshAttendance } = useTFOAttendance();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (form.id) {
                ...
                toast({ title: "TFO worker updated successfully" });
            } else {
                await addWorker(worker);
                toast({ title: "TFO worker added successfully" });
            }
            resetForm();
        } catch (error) {
            toast({
                title: "Error",
                description: "Failed to save TFO worker.",
                variant: "destructive"
            });
        }
    };
    ...
    const editAttendance = async (att: TFOAttendance) => {
        if (confirm("Are you sure you want to edit this attendance record?")) {
            try {
                await removeAttendance(att.id);
    ...
    const handleClearAttendance = async () => {
        if (confirm("Are you sure you want to delete ALL TFO attendance records?")) {
            try {
                await asyncStorage.clearTFOAttendance();
                await refreshAttendance();
                toast({ title: "TFO attendance cleared" });
            } catch (error) {
                toast({
                    title: "Error",
    ...
    // Columns
    const workerColumns = useMemo<ColumnDef<TFOWorker>[]>(() => [
        {
            accessorKey: "name",
            header: ({ column }) => (
    ...
    const attendanceColumns = useMemo<ColumnDef<TFOAttendance>[]>(() => [
        {
            accessorKey: "date",
            header: ({ column }) => (
                <Button variant="ghost" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex justify-end gap-2">
                    <Button size="sm" variant="ghost" onClick={() => editAttendance(row.original)}>
                        <Pencil className="h-4 w-4" />
                    </Button>
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h2 className="text-xl font-semibold">TFO Workers (Attendance Based)</h2>
            ...
            {showForm && (
                <Card>
                    <CardHeader>
                        <CardTitle>{form.id ? "Edit Worker" : "Add New TFO Worker"}</CardTitle>
                    </CardHeader>
                    <CardContent>
                            ...
                            <div>
                                ...
                                {workers.length === 0 ? (
                                    <div className="text-center py-8 text-muted-foreground">
                                        No TFO workers available. Please add workers first.
                                    </div>
                                ) : (
                                                ...
                                                <div key={worker.id} className={`flex items-center justify-between p-3 rounded-lg border transition-colors ${workerData.selected ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
                                                    ...
                                                    <div className="flex gap-4">
                                                        <label className="flex items-center gap-2 cursor-pointer">
                                                            <input type="radio" name={`type-${worker.id}`} value="full" checked={workerData.type === 'full'} onChange={() => updateWorkerType(worker.id, 'full')} disabled={!workerData.selected} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
                                                            <span className={`text-sm ${!workerData.selected ? 'text-gray-400' : 'text-gray-700'}`}>Full Day</span>
                                                        </label>
                                                        <label className="flex items-center gap-2 cursor-pointer">
                                                            <input type="radio" name={`type-${worker.id}`} value="half" checked={workerData.type === 'half'} onChange={() => updateWorkerType(worker.id, 'half')} disabled={!workerData.selected} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
            ...
            <div className="flex flex-col gap-6">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>TFO Workers List</CardTitle>
                    </CardHeader>
                    <CardContent>
